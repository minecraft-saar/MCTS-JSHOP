...
public class MCTSExpansionLandmark implements MCTSExpand{
    
        boolean recursive;
        boolean deadEnd;
        JSJshopVars vars;

        MCTSExpansionLandmark(boolean recursive, boolean deadEnd, JSJshopVars vars){
            ...
            if (node.children.size() == 0) {
                if (t.isPrimitive()) {
                    ...
                    if (ans.isFailure()) {
                        ...
                        node.setCost(node.plan.planCost);
                        node.setDeadEnd(vars);
                        //JSUtil.println("DeadEnd with cost: " + node.getCost(vars));
                        return children;
                    } else {
                        ...
                        child.tState().state().taskLandmarks.removeIf(landmark -> landmark.compare(t));
                        children.add(child);
                        //return children;
                    }
                } else {
